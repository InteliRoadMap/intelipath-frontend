...
export const ActionableListWidget = () => {
  ...
  // Data processing
  const highRecData = Array.isArray(recData) ? recData.filter((item: any) => item.severity?.toUpperCase() === 'HIGH' || item.severity === 'High') : []
  
  const sourceData = activeTab === 'gaps' ? (Array.isArray(data) ? data : []) : highRecData
