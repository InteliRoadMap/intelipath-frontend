...
import { Mail, Lock, Eye, EyeOff, KeyRound, ArrowRight, ArrowLeft, ShieldCheck } from 'lucide-react'
...
export default function ResetPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await authApi.resetPassword({ email: emailFromUrl, otp: otp.trim(), newPassword })
      setSuccess(true)
    } catch (err) {
  ...
  return (
    <div className="w-full">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* OTP Code */}
        <div className="flex flex-col gap-1.5">
          <label htmlFor="reset-otp" className="text-xs font-semibold text-slate-400 tracking-wide">
            OTP Code
          </label>
          <div className="relative">
          ...
          {errors.otp && (
            <span className="text-rose-400 text-xs font-medium pl-1 leading-none">{errors.otp}</span>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {/* New Password */}
          <div className="flex flex-col gap-1.5">
            <label htmlFor="reset-newpass" className="text-xs font-semibold text-slate-400 tracking-wide">
              New Password
            </label>
            <div className="relative">
              <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500">
              ...
              <input
                ...
                className={`w-full text-sm pl-10 pr-10 py-2.5 rounded-xl glass-input ${
                  errors.newPassword ? 'border-rose-500/50 focus:border-rose-500' : ''
                }`}
              />
              <button
                type="button"
            ...
            {errors.newPassword && (
              <span className="text-rose-400 text-xs font-medium pl-1 leading-none">{errors.newPassword}</span>
            )}
          </div>

          {/* Confirm Password */}
          <div className="flex flex-col gap-1.5">
            <label htmlFor="reset-confirm" className="text-xs font-semibold text-slate-400 tracking-wide">
              Confirm Password
            </label>
            <div className="relative">
              <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500">
            ...
            {errors.confirmPassword && (
              <span className="text-rose-400 text-xs font-medium pl-1 leading-none">{errors.confirmPassword}</span>
            )}
          </div>
        </div>
