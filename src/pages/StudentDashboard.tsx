...
import AssessmentModal from "./SkillAssessmentModal"
...
const WelcomeHeader = () => {
  ...
  useEffect(() => {
    void dashboardApi.getRoadmapProgress()
  }, [])
...
const RoadmapProgressWidget = () => {
  ...
  useEffect(() => {
    dashboardApi.getRoadmapProgress().then((res) => setData(res.data))
  }, [])
    ...
    <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-[0_2px_10px_rgba(0,0,0,0.02)] flex flex-col h-full min-h-[260px]">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="bg-[#f8fafc] p-1 rounded-lg flex text-[13px] font-semibold border border-slate-100">
          <button className="bg-[#4fc3f7] text-white px-4 py-1.5 rounded-md shadow-sm">
            Foundations
          </button>
          <button className="text-slate-500 px-4 py-1.5 hover:text-slate-700">
            Advanced
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center pb-2">
        ...
        {data && data.steps && (
          <div className="relative flex justify-between items-start mb-8 px-4">
            ...
            <div
              className="absolute top-[14px] left-8 h-[2px] bg-[#006064] z-0 transition-all"
              style={{ width: "60%" }}
            ></div>
                ...
                >
                  ...
                  {step.status === "current" && (
                    <MapPin
                      size={14}
                      fill="currentColor"
                      className="text-white bg-[#006064] rounded-full p-0.5"
                    />
                  )}
                  {step.status === "locked" && <Lock size={12} />}
                </div>
                <span
                  className={`text-[12px] font-bold ${step.status === "locked" ? "text-slate-300" : step.status === "current" ? "text-[#006064]" : "text-slate-700"}`}
                >
                  {step.title}
                </span>
              </div>
...
const SkillGapsWidget = () => {
  ...
  useEffect(() => {
    dashboardApi.getSkillGaps().then((res) => setData(res.data))
  }, [])
      ...
      <div className="flex-1 space-y-3.5">
        {data && data.length > 0 ? (
          data.map((gap) => (
            <div
              key={gap.id}
              className={`rounded-xl p-4 border ${gap.type === "critical" ? "bg-[#fff5f5] border-[#ffe4e4]" : "bg-[#f0f9ff] border-[#e0f2fe]"}`}
            >
              <div className="flex items-center justify-between mb-1.5">
                <span
                  className={`text-[9px] font-bold tracking-wider uppercase ${gap.type === "critical" ? "text-rose-600" : "text-[#0284c7]"}`}
                >
                  {gap.type === "critical"
                    ? "CRITICAL GAP"
                    : "MARKET REQUIREMENT"}
                ...
                <span
                  className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${gap.type === "critical" ? "bg-[#fee2e2] text-rose-600" : "bg-[#bae6fd] text-[#0369a1]"}`}
                >
                  {gap.severity}
                </span>
              </div>
...
const MentorFeedbackWidget = () => {
  const [data, setData] = useState<MentorFeedback[] | null>(null)
  useEffect(() => {
    dashboardApi.getMentorFeedback().then((res) => setData(res.data))
  }, [])
            ...
            <div key={feedback.id} className="relative pl-4">
              ...
              <div className="bg-[#f8fafc] rounded-xl rounded-l-none border border-slate-100 p-4">
                <div className="flex items-center justify-between mb-2">
                  <h4 className="text-[14px] font-bold text-[#006064]">
                    {feedback.name}
                  </h4>
...
const SkillComparisonWidget = () => {
  const [data, setData] = useState<SkillComparison[] | null>(null)
  useEffect(() => {
    dashboardApi.getSkillComparison().then((res) => setData(res.data))
  }, [])
    ...
    <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-[0_2px_10px_rgba(0,0,0,0.02)] flex flex-col h-full min-h-[300px]">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-4 text-[10px] font-bold text-slate-600 uppercase tracking-wide">
          ...
          <span className="flex items-center gap-1.5">
            <span className="w-2.5 h-2.5 rounded-full bg-[#e0f2fe]"></span>{" "}
            Target
          </span>
        </div>
      </div>
          ...
          data.map((skill) => (
            <div key={skill.id}>
              ...
              <div className="h-2 w-full bg-[#f1f5f9] rounded-full overflow-hidden flex relative">
                {/* Target background */}
                <div
                  className="absolute left-0 top-0 bottom-0 bg-[#e0f2fe] z-0"
                  style={{ width: `${skill.target}%` }}
                ></div>
                {/* Current bar */}
                <div
                  className="absolute left-0 top-0 bottom-0 bg-[#006064] z-10 rounded-r-full"
...
const AiMentorHistoryWidget = () => {
  const [data, setData] = useState<AiHistoryItem[] | null>(null)
  useEffect(() => {
    dashboardApi.getAiHistory().then((res) => setData(res.data))
  }, [])
      ...
      <div className="flex-1 space-y-3">
        {data && data.length > 0 ? (
          data.map((item, index) => (
            <div
              key={item.id}
              className={`p-4 rounded-xl border ${index === 1 ? "border-[#00838f] bg-white border-l-[3px]" : "border-slate-100 bg-[#f8fafc]"}`}
            >
              <span className="text-[10px] font-bold text-slate-500 mb-1 block uppercase">
                {item.tag}
              </span>
              <h4 className="text-[14px] font-bold text-slate-900 mb-1 truncate">
                {item.title}
              </h4>
...
const BridgeGapsWidget = () => {
  ...
  useEffect(() => {
    dashboardApi.getRecommendations().then((res) => setData(res.data))
  }, [])

  return (
    <div className="w-full mt-8">
      <h3 className="font-bold text-slate-900 text-[17px] mb-4">
        Bridge Your Gaps - Priority Learning
      </h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 min-h-[200px]">
        {data && data.length > 0 ? (
          data.map((item) => (
            <div
              key={item.id}
              className="bg-[#f8fafc] border border-slate-100 rounded-2xl p-6 flex flex-col h-full"
            >
              ...
              <h4 className="font-bold text-slate-900 text-[16px] mb-2">
                {item.title}
              </h4>
...
const MarketDemandWidget = () => {
  const [data, setData] = useState<MarketDemand | null>(null)
  useEffect(() => {
    dashboardApi.getMarketDemand().then((res) => setData(res.data))
  }, [])
        ...
        <>
          <div className="flex items-center gap-3 mb-6 mt-2">
            ...
            <div className="flex flex-col justify-center">
              <span className="text-[#00838f] text-[10px] font-bold">
                YoY Growth
              </span>
          ...
          <div className="flex-1 flex flex-col justify-end">
            ...
            <p className="text-center text-[8px] font-bold tracking-widest uppercase text-slate-800">
      ...
      <nav className="bg-white border-b border-slate-200 px-8 py-3.5 flex items-center justify-between sticky top-0 z-40">
        <div className="flex items-center gap-12">
          ...
          <div className="hidden md:flex items-center gap-8 text-[13px] font-bold text-slate-500">
            <a
              href="#"
              className="flex items-center gap-2 text-[#00838f] border-b-[3px] border-[#00838f] py-4 -mb-3.5 transition-colors"
            >
        ...
        <div className="flex items-center gap-6 relative">
          ...
          {/* User Profile Area */}
          <div
            className="flex items-center gap-3 pl-0 sm:pl-6 sm:border-l border-slate-200 cursor-pointer group relative"
            onClick={() => setIsDropdownOpen(!isDropdownOpen)}
          >
            <div className="text-right hidden sm:block">
              <p className="text-[13px] font-bold text-slate-900 group-hover:text-[#00838f] transition-colors">
              ...
              <>
                ...
                <div className="absolute right-0 mt-3 w-48 bg-white rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.08)] border border-slate-100 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                  <button 
                    ...
                    className="w-full text-left px-4 py-2.5 text-[14px] text-slate-700 hover:bg-slate-50 transition-colors flex items-center gap-3 font-medium"
                  >
